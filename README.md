...
默认菜品图统一放在 `dishes/default/`，家庭自定义菜品图放在 `dishes/families/{家庭ID}/`，头像统一放在 `avatars/`。不再按月份分目录。家庭默认使用系统菜谱；开启自定义后，只修改当前家庭的菜，分类仍使用系统里已有的菜品类型。

```http
GET /api/files/dishes/default/{文件名}
```
