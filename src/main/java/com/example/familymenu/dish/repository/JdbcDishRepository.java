...
public class JdbcDishRepository implements DishRepository {
    ...
    public List<Dish> findAvailable(String category) {
        String sql = "SELECT id, name, category, image_url, available, sort "
    ...
    public Optional<Dish> findById(Long id) {
        List<Dish> dishes = jdbcTemplate.query(
                "SELECT id, name, category, image_url, available, sort FROM dishes WHERE id = ?",
        ...
        if (dish.getId() == null) {
            ...
            jdbcTemplate.update(connection -> {
                PreparedStatement statement = connection.prepareStatement(
                        "INSERT INTO dishes (name, category, image_url, available, sort) "
                                + "VALUES (?, ?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
                statement.setString(1, dish.getName());
                statement.setString(2, dish.getCategory());
                statement.setString(3, dish.getImageUrl());
                statement.setBoolean(4, dish.isAvailable());
                statement.setInt(5, dish.getSort());
                return statement;
            }, keyHolder);
            return new Dish(keyHolder.getKey().longValue(), dish.getName(), dish.getCategory(),
                    dish.getImageUrl(), dish.isAvailable(), dish.getSort());
        }
        jdbcTemplate.update("UPDATE dishes SET name = ?, category = ?, image_url = ?, available = ?, sort = ? "
                        + "WHERE id = ?", dish.getName(), dish.getCategory(), dish.getImageUrl(),
